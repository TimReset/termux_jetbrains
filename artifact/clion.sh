...
if [ -z "$JRE" ] && [ -s "${CONFIG_HOME}/JetBrains/CLion2023.2/clion.jdk" ]; then
  USER_JRE=$(cat "${CONFIG_HOME}/JetBrains/CLion2023.2/clion.jdk")
...
if [ -z "$JAVA_BIN" ] || [ ! -x "$JAVA_BIN" ]; then
  message "No JRE found. Please make sure \$CLION_JDK, \$JDK_HOME, or \$JAVA_HOME point to valid JRE installation."
  exit 1
fi
...
# shellcheck disable=SC2154
if [ -n "$CLION_VM_OPTIONS" ] && [ -r "$CLION_VM_OPTIONS" ]; then
  # 1. $<IDE_NAME>_VM_OPTIONS
  VM_OPTIONS_FILE="$CLION_VM_OPTIONS"
else
  # 2. <IDE_HOME>/bin/[<os>/]<bin_name>.vmoptions ...
  if [ -r "${IDE_BIN_HOME}/clion64.vmoptions" ]; then
    VM_OPTIONS_FILE="${IDE_BIN_HOME}/clion64.vmoptions"
  else
    test "${OS_TYPE}" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
    if [ -r "${IDE_BIN_HOME}/${OS_SPECIFIC}/clion64.vmoptions" ]; then
      VM_OPTIONS_FILE="${IDE_BIN_HOME}/${OS_SPECIFIC}/clion64.vmoptions"
    fi
  fi
  ...
  elif [ -r "${CONFIG_HOME}/JetBrains/CLion2023.2/clion64.vmoptions" ]; then
    USER_VM_OPTIONS_FILE="${CONFIG_HOME}/JetBrains/CLion2023.2/clion64.vmoptions"
  fi
fi
...
CLASS_PATH="$CLASS_PATH:$IDE_HOME/lib/junit5.jar"
